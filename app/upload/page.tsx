    ...
    <div style={{ maxWidth: 700, margin: "0 auto", display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      ...
      <div className="animate-in delay-2" style={{ display: "grid", gap: "1rem" }}>
        <div>
          ...
          <input className="input" placeholder="e.g. models/llama-3b.gguf"
            value={name} onChange={e => setName(e.target.value)} />
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
          <div>
            <label className="label">TTL</label>
            <select className="select" style={{ width: "100%" }} value={ttl}
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: "var(--text-3)", marginTop: 3 }}>
              {parseFloat(price || "0") > 0
                ? `${Math.round(parseFloat(price) * 1e8).toLocaleString()} octas`
                : "Free reads"}
            </div>
          </div>
        </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
        <button className="btn btn-primary"
          style={{ width: "100%", padding: "0.9rem", fontSize: 14 }}
          onClick={upload}
        ...
        {error && (
          <div style={{
            ...
            borderRadius: 8, padding: "0.75rem 1rem", fontSize: 13, color: "#ff5050",
      ...
      {result && (
        <div className="card-glow animate-in" style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
          ...
          {[
            ...
            ["Size",     result.sizeBytes ? fmt(Number(result.sizeBytes)) : ""],
          ].map(([k, v]) => v && (
            <div key={k} style={{ display: "flex", justifyContent: "space-between", fontSize: 12, gap: "1rem" }}>
              ...
              <span style={{ fontFamily: "var(--font-mono)", fontSize: 11, textAlign: "right", wordBreak: "break-all" }}>{v}</span>
            </div>
          ))}
          {result.explorerUrl && (
            <a href={result.explorerUrl} target="_blank" rel="noreferrer"
              className="btn btn-secondary"
              style={{ textDecoration: "none", textAlign: "center", marginTop: 4 }}>
